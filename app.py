...
def buscar_jogo():
    id_jogo = 0 
    jogo = executar_query("SELECT * FROM jogos WHERE id = ? ", id_jogo, fetch=True)
    if jogo: 
        return jsonify(dict(jogos[0])), 200 
...
def adicionar_jogo():
    jogos = ("Minecraft", "Mojang Studios", 2011, 89.90)
...
@app.route('/atualizarjogo', methods=['PUT'])

def atualizar_jogo(id): 
    id_jogo = 0
    dados = request.get_json()
    
    existe = executar_query("SELECT id FROM produtos WHERE id = ?", id_jogo, fetch=True)
    if not existe:
        return jsonify({"erro": "Jogo não encontrado"}), 404
...
@app.route('/deletarjogo', methods=['DELETE'])
